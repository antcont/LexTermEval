...
for test_ref in test:
    ...
    #  list of tuples tuple -> (id, src, ref, hyp, src_lemma, ref_lemma, hyp_lemma)
...
identified_terms = []
matched_after_split = []    # see which additional matches have been found after compound-spliting

#  FINAL ANNOTATION DATA
# list of tuples (sentenceID, src, ref, hyp, src_l, ref_l, hyp_l, conceptID, terminology_entry, src_term, hyp_term, C/W)
annotated_data = []

#  in order to compute the term rate
counter_correct = 0
counter_wrong = 0

#  adding all Italian terms to the Italian PhraseMatcher
id_terms = tb_string2dict(terms)            # converting termlist to dictionary
for id, (it, de) in id_terms.items():
    ...
    matcher_it.add(str(id), pattern_it)   # adding Italian terms to PhraseMatcher, with respective conceptID
...
for (id, src, ref, hyp, src_lemma, ref_lemma, hyp_lemma) in test_:  # iterating over each sentence-tuple in the test-set
    ...
    doc_it = nlp_it.make_doc(src_lemma)         # Str to Doc
    doc_it_or = nlp_it.make_doc(src)            # Str to Doc
    ...
    # filtering out duplicates or overlaps, greedily.
    ...
    for span in matches_it:                      # iterating over each single term match on the Italian source sentence
        start = span.start
        end = span.end
        match_id = span.label
        ...
        matcher_de.add(concept_id, pattern_de)                      # adding DE term(s) to the German PhraseMatcher
        ...
        if not matches_de:   # if no match is found in the German reference sentence, I retry after splitting compounds
            splitter = Splitter()       # instantiating the compound splitter
            ...
            for token in doc_de:
                ...
                if hyp_split[0] > 0.7:          # tweak threshold; a lower threshold allows for higher recall
                                                # without influencing precision (we are looking for one set of terms,
                                                # therefore there's no risk of introducing noise)
            ...
            doc_de = nlp_de.make_doc(split_text)  # overwriting the original Doc with the one containing split compounds
            doc_de_or = nlp_de.make_doc(split_text)  # TODO: CHECK THIS: by doing so, I am overwriting the actual original non-lemmatised DE sentence,
                                                     #  and, as a consequence, I am not annotating the actual term from the original sentence, but the lemmatised form
                                                     #  It's actually not a problem, but still, it is a small inconsistency

            # now retrying to match on the German reference sentence with split compounds
            matches_de = matcher_de(doc_de, as_spans=True)  # checking for term matches in the German reference sentence
            matches_de = filter_spans(matches_de)           # filtering overlapping matches (greedy)
        ...
        # here matches_de is the German match from either the first search or the search after compound splitting
        for span in matches_de:  # iterating over each single German term match in the reference sentence
            start = span.start
            end = span.end
            match_id = span.label
            ...
            # now checking for matches in the German hypothesis sentence from ModernMT (baseline or adapted)
            ...
            if not matches_de:
                ...
                for token in doc_hyp:
                    split_token = splitter.split_compound(token.text)  # output is a list of tuples (score, word, word)
                    hyp_split = split_token[0]  # considering only the first splitting hypothesis
                    if hyp_split[0] > 0.7:  # tweaking threshold
                        # TODO: remember to lemmatise compounds when doing official tests (much slower)
                        #split_text.append(lemmatise(hyp_split[1], "de"))  # lemmatising and appending first word of compound
                        #split_text.append(lemmatise(hyp_split[2], "de"))  # lemmatising and appending second element of compound
                        split_text.append(hyp_split[1])  # appending first word of compound (faster, no lemmatisation)
                ...
                doc_hyp_or = nlp_de.make_doc(split_text) # TODO: check this; same as above, the matched term will not be
                                                         #  in its original form, but lemmatised. No big problem

                # now retrying to match on German hyp sentence with split compounds
            ...
            for span in matches_de:  # iterating over each single term match on the German hyp sentence
                counter_correct += 1
                start = span.start
                end = span.end
                match_id = span.label
                ...
                # computing Levenshtein edit distance to pair matched term with term in termbase
                lev_dist = []
                for deTerm in De:
                ...
                lev_dist.sort(key=operator.itemgetter(0))   # sorting by lowest Levenshtein distance
